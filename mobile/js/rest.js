$(document).ready(function() {
    ...
    function branchListClick() {
        ...
        $('.restaurant').on('click', '.list1 ul li',
            function() {
                resetStyles();
                let ind = $(this).index();
                $('.branch img').attr('src', './images/sub3/' + map[ind].imgsrc).fadeIn('fast');
        ...
        $('.restaurant').on('click', '.list2 ul li',
            function() {
                resetStyles();
                let ind = $(this).index();
                $('.branch img').attr('src', './images/sub3/' + map2[ind].imgsrc).fadeIn('normal');
    ...
    // AJAX 요청으로 데이터를 로드한 후, click 이벤트를 초기화
    $.ajax({
        url: './data/rest.json',
        dataType: 'json',
        success: function(data) {
            ...
            // 비동기로 추가된 요소들에 대해 click 이벤트를 초기화
            branchListClick();
        }
    });

    // 초기 페이지 로드 시 click 이벤트 초기화~
    branchListClick();
});